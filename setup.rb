...
puts "Now Installing Base..."
puts "="*ROW_SIZE
puts ""

puts_section "Checking Package Dependencies..." do
  pkg_exist = true
  [["bundle","Bundler"], ["psql", 'Postgresql'],["convert","ImageMagick 6.5+"]].each do |item|
...
puts_section "Configure" do
  %w(database).each do |fname|
    `cp config/#{fname}.yml.sample config/#{fname}.yml`
  end
end
...
puts ""
puts "Base Successfully Installed."
